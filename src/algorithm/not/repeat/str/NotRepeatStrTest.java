...
/**
 * 求a-z字符串中最大无重复子串的长度
 * 解题思路：计算所有以第i个位置元素结尾的最大子串长度，记录下来
 * 这样一来，可以求出以所有位置元素结尾的最大子串长度，最后的结果就是在这些值中取最大的即可。
 * 求第i个位置的最大长度时，有两个影响因素，一个是这个位置上元素上次出现的位置，一个是第i-1位置上求出来的最大长度
 * 这两个因素取最小值即为这个位置上的最大子串长度
 *
 * @Author zp
